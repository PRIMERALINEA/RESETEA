...
Conservación:
Los datos se conservan mientras mantengas tu cuenta activa.

Derechos:
Tienes derecho a acceder, rectificar, suprimir, oponerte y limitar el tratamiento de tus datos.
Contacto: reseteaacademica@gmail.com

Seguridad:
Todos los datos se almacenan en servidores europeos con cifrado en tránsito y en reposo.
...
export default function RegistroIndividual() {
  ...
  const handleRegistro = async () => {
    ...
    if (!email || !password || !curso) {
      setError('Por favor rellena todos los campos'); setLoading(false); return
    }
    if (!aceptaPolitica) {
      setError('Debes aceptar la política de protección de datos'); setLoading(false); return
    }
    ...
    try {
      ...
      // 4. Redirigir directamente al pago sin esperar email
  ...
  const canSubmit = email && password && confirmPassword && curso && aceptaPolitica
    ...
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-teal-200 text-sm mt-1">Acceso individual · 29,99 € / curso</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 space-y-4">
          ...
          <p className="text-xs text-slate-500 -mt-2">Usa cualquier email — no necesitas email de centro</p>
