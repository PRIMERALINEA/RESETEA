        ...
        <div className="space-y-2 mb-5">
          {ESTADOS.map(estado => (
            <button key={estado.valor} onClick={() => setSelected(estado)}
              ...
              <span className="font-bold text-sm flex-1" style={{ color: selected?.valor === estado.valor ? estado.color : '#334155' }}>{estado.label}</span>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <button onClick={() => setMostrarSemaforo(true)}
          ...
          {estadoActual ? (
            ...
            <>
              <p className="text-sm font-medium" style={{ color: 'rgba(255,255,255,0.7)' }}>Toca para indicar tu estado</p>
              <div className="flex gap-1.5">
      ...
      <AnimatePresence>
        {quinc && !mostrarQuincenal && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="mb-4 rounded-2xl p-4 flex items-center gap-3 cursor-pointer transition-all hover:opacity-90"
